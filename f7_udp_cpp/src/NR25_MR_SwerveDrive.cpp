...
// IPアドレスとポートの指定
std::string udp_ip =
    "192.168.8.215"; // 送信先IPアドレス、宛先マイコンで設定したIPv4アドレスを指定
...
public:
  PS4_Listener(const std::string &ip, int port)
    ...
    subscription_bool = this->create_subscription<std_msgs::msg::Bool>(
        "/backward_signal", 10,
        std::bind(&PS4_Listener::bool_callback, this, std::placeholders::_1));
...
public:
  Servo_Deg_Publisher() : Node("servo_deg_publisher") {
    // Publisherの作成
...
public:
  Params_Listener() : Node("nr25_servo_cal_listener") {
